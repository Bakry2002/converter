...
class ImageConverter extends types_1.Converter {
    ...
    async preConvert() { }
    async execute() {
        console.log(`${process.env.NODE_ENV === 'development' ? 'convert' : 'magick'}${this.inputOptions()} ${this.input()} ${this.outputOptions()} ${this.output()}`);
        console.log('===============================');
        await exec(`${process.env.NODE_ENV === 'development' ? 'convert' : 'magick'}${this.inputOptions()} ${this.input()} ${this.outputOptions()} ${this.output()}`, { cwd: this.cwd });
    }
    async postConvert() { }
    async preRead() { }
    async read() {
