...
// EDIT: change the converter to accept an array of buffers instead of a single buffer
//* =========================
...
const buildConverter = (from, to, params) => {
    const converter = async (buffers) => {
        ...
        const data = await (0, promises_1.readFile)(`/tmp/${file}.${(0, mime_types_1.extension)(to)}`); // return the converted file
        return [data];
    };
    converter.from = from;
    converter.to = to;
